...
public final class ElytraBehavior implements Helper {
    ...
    private void onTick0() {
        ...
        if (this.remainingFireworkTicks > 0) this.remainingFireworkTicks--;
        if (this.remainingSetBackTicks > 0) this.remainingSetBackTicks--;
        if (!this.getAttachedFirework().isPresent()) this.minimumBoostTicks = 0;
    ...
    public void tick() {
        if (this.pathManager.getPath().isEmpty()) return;
        ...
        for (int relaxation = 0; relaxation < 3; relaxation++) {
            ...
            for (int i = Math.min(playerNear + 20, path.size() - 1); i >= minStep; i--) {
                ...
                for (final Pair<Vec3, Integer> candidate : candidates) {
                    ...
                    if (landingMode) dest = dest.add(0.5, 0.5, 0.5);
                    ...
                    if (augment != 0) {
                        ...
                        if (start.distanceTo(dest) < 40) {
                            ...
                            if (!this.clearView(dest, path.getVec(i), false)) continue;
                        }
                    }
    ...
    private void tickUseFireworks(final Vec3 start, final Vec3 goingTo, final boolean isBoosted, final boolean forceUseFirework) {
        if (this.remainingSetBackTicks > 0) return;
    ...
    private final class SolverContext {
        ...
        public boolean equals(Object o) {
            ...
            if (o == null || o.getClass() != SolverContext.class) return false;
    ...
    private static final class FireworkBoost {
        ...
        public boolean equals(Object o) {
            if (this == o) return true;
            if (o == null || o.getClass() != FireworkBoost.class) return false;
            FireworkBoost other = (FireworkBoost) o;
            if (!this.isBoosted() && !other.isBoosted()) return true;
    ...
    public static boolean isFireworks(final ItemStack itemStack) {
        ...
        // 1.21.4 Fix: Use DataComponents instead of getTagElement
    ...
    private static FloatArrayList pitchesToSolveFor(final float goodPitch, final boolean desperate) {
        ...
        for (float pitch = goodPitch; pitch <= maxPitch; pitch++) pitchValues.add(pitch);
        for (float pitch = goodPitch - 1; pitch >= minPitch; pitch--) pitchValues.add(pitch);
        return pitchValues;
    }
    ...
    private PitchResult solvePitch(final SolverContext context, final Vec3 goal, final int relaxation,
        ...
        while (pitches.hasNext()) {
            ...
            final List<Vec3> displacement = this.simulate(
                    context, goalDelta, pitch, ticks, ticksBoosted, ticksBoostDelay
            );
            ...
            for (int x = xmin; x < xmax; x++) {
                for (int y = ymin; y < ymax; y++) {
                    for (int z = zmin; z < zmax; z++) {
                        if (!this.passable(x, y, z, context.ignoreLava)) return null;
                    }
                }
            }
    ...
    private void trySwapElytra() {
        if (!Baritone.settings().elytraAutoSwap.value || !invTransactionQueue.isEmpty()) return;
    ...
    void logVerbose(String message) {
        if (Baritone.settings().elytraChatSpam.value) logDebug(message);
    }
}
